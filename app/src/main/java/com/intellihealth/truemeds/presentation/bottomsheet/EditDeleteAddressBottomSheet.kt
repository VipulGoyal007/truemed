...
class EditDeleteAddressBottomSheet  : BottomSheetDialogFragment(){
    ...
    fun setManageAddressViewModel(viewModel:ManageAddressViewModel, isEditClick:Boolean, isDeleteClick:Boolean) {
        ...
        binding.deleteReminder.setOnClickListener {
            if(isDeleteClick) {
                if(::viewModel.isInitialized) {
                    viewModel.deleteAddressApiCalling()
                }
                ...
                if(::viewModel.isInitialized) {
                    viewModel.showMessage.postValue(Event(MessageConstant(MESSAGES.DISCARD_EDIT_ADDRESS_CHANGES)))
                }
            }
            dismiss()
        }
